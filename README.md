...
chaque collection est indexée (chunking + embeddings + Qdrant), et un agent de recherche
...
Quatre services applicatifs, plus l'infrastructure (Postgres, Redis, Qdrant, Keycloak, RustFS) :
...
| [`backend/`](backend/README.md) | API FastAPI (BFF) : auth Keycloak, CRUD collections/documents/runs, seul service à parler à Postgres/Qdrant/RustFS/au hub LLM | [backend/README.md](backend/README.md) |
...
Les deux workers ne parlent jamais directement à Postgres/Qdrant/RustFS : ils passent uniquement
...
make up        # lance toute la stack en Docker (frontend, backend, workers, Postgres, Redis, Keycloak, Qdrant, RustFS)
```
