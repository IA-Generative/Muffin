...
def test_health_is_healthy_when_all_dependencies_are_up(client):
    with (
        ...
        patch("app.routers.health.meilisearch_connector") as mock_meilisearch,
        patch("app.routers.health.db_connector") as mock_db,
    ):
        mock_redis.get_health.return_value = Health(name="redis", status="healthy")
        mock_meilisearch.get_health.return_value = Health(name="meilisearch", status="healthy")
    ...
    assert body["dependencies"] == [
        ...
        {"name": "meilisearch", "status": "healthy", "extras": None},
